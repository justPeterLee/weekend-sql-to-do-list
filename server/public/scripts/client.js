...
function onReady(){
    ...
    // complete div
    $('.task-output').on('click', '.completed', toggleCheck)
...
function renderTask(task){
    ...
    `).prependTo('.task-output')
        ...
        else if(task[i].completion === 1){
            $(`
        <div id="task-display" data-id=${task[i].id}>
            <div class="completed"><p class="completed-task">Completed</p></div>
            <div id="task-container" class="blur">
            <div class="word-display">
                <div class="title-display">
                    <div class="diff-display"></div>
                    <h1 class="task-title">${task[i].task}</h1>
